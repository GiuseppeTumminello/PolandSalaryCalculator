...
public class SalaryCalculatorMenu {
    ...
    public void statistics(BigDecimal grossMonthlySalary) {
        JobsCategory jobCategory = salaryCalculatorInputValidation.surveyInput();
        if (jobCategory != null) {
            printerCalculator.printJobTitle(jobCategory);
            String jobTitle = salaryCalculatorInputValidation.jobTitleValidator(jobCategory);
