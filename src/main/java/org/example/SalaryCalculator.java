...
public class SalaryCalculator {
    ...
    public BigDecimal getHealthNfzTotal() {
        return this.gross.subtract(getZusTotal()).multiply(BigDecimal.valueOf(healthNFZRate)).divide(BigDecimal.valueOf(100.0d), RoundingMode.HALF_UP).setScale(2, RoundingMode.HALF_EVEN);
    ...
    public BigDecimal getAdvancePaymentTotal() {
        if (this.getAnnualSum().compareTo(BigDecimal.valueOf(120_000.00)) < 0) {
            this.taxRate = 17;
            return (this.gross.subtract((getHealthNfzTotal().add(getZusTotal())))).multiply(BigDecimal.valueOf(8.32d)).divide(BigDecimal.valueOf(100.0d), RoundingMode.HALF_UP).setScale(2, RoundingMode.HALF_EVEN);

        } else {
            this.taxRate = 32;
            return (this.gross.subtract((getHealthNfzTotal().add(getZusTotal())))).multiply(BigDecimal.valueOf(14.32d)).divide(BigDecimal.valueOf(100.0d), RoundingMode.HALF_UP).setScale(2, RoundingMode.HALF_EVEN);
        }
    }


    public BigDecimal getNet() {
        return this.gross.subtract(getZusTotal().add(getHealthNfzTotal()).add(getAdvancePaymentTotal())).setScale(2, RoundingMode.HALF_EVEN);

    }

    public BigDecimal getZusPension() {
        return this.gross.multiply(BigDecimal.valueOf(zusPensionRate)).divide(BigDecimal.valueOf(100.0d), RoundingMode.HALF_UP).setScale(2, RoundingMode.HALF_EVEN);
    }

    public BigDecimal getZusDisability() {
        return this.gross.multiply(BigDecimal.valueOf(zusDisabilityRate)).divide(BigDecimal.valueOf(100.0d), RoundingMode.HALF_UP).setScale(2, RoundingMode.HALF_EVEN);


    }

    public BigDecimal getZusSickness() {
        return this.gross.multiply(BigDecimal.valueOf(zusSicknessRate)).divide(BigDecimal.valueOf(100.0d), RoundingMode.HALF_UP).setScale(2, RoundingMode.HALF_EVEN);
