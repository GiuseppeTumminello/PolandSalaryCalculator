...
public enum SalaryCalculatorEnum {
    ...
    HEALTH(gross -> gross.subtract(TOTAL_ZUS.operator.apply(gross)).multiply(BigDecimal.valueOf(0.09)).setScale(2, RoundingMode.HALF_EVEN), "Health amount: "),
    ...
    TAX(gross -> {
        if (GROSS_YEARLY.getOperator().apply(gross).compareTo(BigDecimal.valueOf(120000)) < 0)
            return gross.subtract(TOTAL_ZUS.operator.apply(gross)).subtract(HEALTH.operator.apply(gross)).multiply(BigDecimal.valueOf(0.0832)).setScale(2, RoundingMode.HALF_EVEN);
        else return gross.multiply(BigDecimal.valueOf(0.1432)).setScale(2, RoundingMode.HALF_EVEN);
    }, "Tax amount: "),
