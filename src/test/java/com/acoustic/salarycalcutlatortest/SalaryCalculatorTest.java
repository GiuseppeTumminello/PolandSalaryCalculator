package com.acoustic.salarycalcutlatortest;
...
class SalaryCalculatorTest {
    ...
    void getZusTotal(BigDecimal input, BigDecimal expected) {
        //Using AssertJ
        assertThat(SalaryCalculator.TOTAL_ZUS.getOperator().apply(input)).isEqualTo(expected);
    }

    @ParameterizedTest
    @CsvSource({"6000, 465.97", "7000, 543.63", "15891.68, 1234.16"})
    void getHealthNfzTotal(BigDecimal input, BigDecimal expected) {
        assertThat(SalaryCalculator.HEALTH.getOperator().apply(input)).isEqualTo(expected);
    ...
    void getAdvancePaymentTotal(BigDecimal input, BigDecimal expected) {
        assertThat(SalaryCalculator.TAX.getOperator().apply(input)).isEqualTo(expected);
    }
    ...
    void getZusPension(BigDecimal input, BigDecimal expected) {
        assertThat(SalaryCalculator.PENSION_ZUS.getOperator().apply(input)).isEqualTo(expected);
    ...
    void getZusDisability(BigDecimal input, BigDecimal expected) {
        assertThat(SalaryCalculator.DISABILITY_ZUS.getOperator().apply(input)).isEqualTo(expected);
    }

    @ParameterizedTest
    @CsvSource({"6000, 147.00", "7000, 171.50", "15891.68, 389.35"})
    void getZusSickness(BigDecimal input, BigDecimal expected) {
        assertThat(SalaryCalculator.SICKNESS_ZUS.getOperator().apply(input)).isEqualTo(expected);
    }

    @ParameterizedTest
    @CsvSource({"6000, 72000.00", "7000, 84000.00", "15891.68, 190700.16"})
    void getYearlyGross(BigDecimal input, BigDecimal expected) {
        assertThat(SalaryCalculator.GROSS_YEARLY.getOperator().apply(input)).isEqualTo(expected);
    }

    @ParameterizedTest
    @CsvSource({"6000, 4319.44", "7000, 5039.35", "15891.68, 10691.81"})
    void getNet(BigDecimal input, BigDecimal expected) {
        assertThat(SalaryCalculator.NET.getOperator().apply(input)).isEqualTo(expected);
    ...
    void getYearlyNet(BigDecimal input, BigDecimal expected) {
        assertThat(SalaryCalculator.NET_YEARLY.getOperator().apply(input)).isEqualTo(expected);
    }
